  ...
  <div class="page-container">
    ...
    <div v-for="m in moments" :key="m.id" class="glass-card" style="margin-bottom:12px">
      ...
      <div v-if="m.images?.length" style="display:flex;gap:6px;flex-wrap:wrap;margin-bottom:8px">
        <el-image v-for="img in m.images" :key="img.thumb" :src="img.thumb||img.url" style="width:100px;height:100px;border-radius:8px" fit="cover" :preview-src-list="m.images.map(i=>i.url||i.thumb)" />
      </div>
    ...
    <el-dialog v-model="showPost" title="发布动态" width="500px">
    ...
    <el-dialog v-model="showComment" title="评论" width="500px">
      <div v-for="c in comments" :key="c.id" style="padding:8px 0;border-bottom:1px solid #eee;font-size:14px"><strong>{{ c.user?.nickname||c.user?.username }}:</strong> {{ c.content }}</div>
