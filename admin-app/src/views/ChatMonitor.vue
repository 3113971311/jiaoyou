<template>
  <div style="padding:24px">
    ...
    <el-input v-model="search" placeholder="搜索用户名..." style="width:240px;margin-bottom:12px" @keyup.enter="load" />
    ...
    <el-dialog v-model="showMsgs" title="消息记录" width="700px">
      <div v-for="m in messages" :key="m.id" style="padding:8px 0;border-bottom:1px solid #eee">
        <strong>{{ m.sender_id }}:</strong>
        <span v-if="m.content_type==='text'">{{ m.content }}</span>
        <el-image v-else-if="m.image_url" :src="m.image_url" style="width:120px;height:120px;border-radius:8px" fit="cover" />
        <small style="color:#999;margin-left:12px">{{ new Date(m.created_at).toLocaleString() }}</small>
      </div>
    </el-dialog>
  </div>
</template>
