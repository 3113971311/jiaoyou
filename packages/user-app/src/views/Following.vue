<template>
  <div class="page">
    <van-nav-bar title="我的关注" left-arrow fixed placeholder @click-left="$router.back()" />
    ...
    <van-empty v-else description="暂无关注" />
  </div>
</template>
...
const list = ref<any[]>([]);
onMounted(async () => {
  const res = await client.get('/follow/following');
  list.value = res.data.list || [];
});
</script>

<style scoped>.page { background: #f7f8fa; min-height: 100vh; padding-top: 10px; }</style>
